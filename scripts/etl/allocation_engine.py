...
def identify_shared_services(conn):
    ...
    cursor.execute("""
        ...
        WHERE 
            ...
            OR cost_center ILIKE '%PLATFORM%'
            OR cost_center ILIKE '%SHARED%'
            OR cost_center ILIKE '%GLOBAL%'
    """)
...
def apply_allocation_rules(conn):
    ...
    for app_id, app_name, cost_center, sector_name in shared_services:
        print(f"  Allocating costs for: {app_name} (Cost Center: {cost_center})")
        
        for rule_id, method, service_code in active_rules:
            if cost_center and service_code in cost_center:
