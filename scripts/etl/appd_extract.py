...
def generate_usage_data_from_api(conn, controller, account, client_id, client_secret, account_id, app_id_map):
    ...
    if usage_data is None:
        # API request failed - terminate ETL
        print("")
        print("=" * 80)
        print("❌ CRITICAL ERROR: AppDynamics Licensing API Unavailable")
        print("=" * 80)
        print("")
        print("The ETL pipeline requires real license usage data from AppDynamics.")
        print("The Licensing API v1 is currently unavailable or not accessible.")
        print("")
        ...
        print("   Run: ./scripts/utils/test_all_prod_controllers.sh")
        ...
        print("   - docs/CLIENT_REQUIREMENTS_DETAILED.md")
        print("   - docs/LICENSING_API_MANUAL_TEST_GUIDE.md")
        print("   - docs/PIPELINE_TROUBLESHOOTING.md")
        print("")
        print("=" * 80)
        print("ETL pipeline terminated. Cannot proceed without real license data.")
        print("=" * 80)
        print("")
        cur.close()
        raise Exception("AppDynamics Licensing API unavailable - ETL terminated")
