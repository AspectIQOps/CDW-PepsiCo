...
def generate_chargeback(conn):
    ...
    print(f"  ℹ️  Processing chargeback for {enriched_count} enriched applications")
    ...
    print(f"\n  H-Code Coverage:")
    print(f"    • Apps with H-code: {h_code_stats[1]}/{h_code_stats[0]} ({h_code_coverage:.1f}%)")
    print(f"    • Apps without H-code: {h_code_stats[2]}")
    
    if h_code_coverage < 90:
        print(f"  ⚠️  H-code coverage below 90% target")
...
def refresh_dashboard_views(conn):
    ...
    try:
        ...
        if view_count > 0:
            ...
            print("  ℹ️  No materialized views configured yet")
...
def generate_summary_report(conn):
    ...
    print(f"  • H-code coverage: {h_code_stats[1]}/{h_code_stats[0]} ({h_code_pct:.1f}%)")
    
    if h_code_pct >= 90:
        print(f"  ✅ Ready for production chargeback")
    else:
        print(f"  ⚠️  Below 90% target - PepsiCo should populate H-codes")
