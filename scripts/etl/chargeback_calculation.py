...
Aggregates license costs into monthly chargeback records by sector and Cost Center
...
- Aggregates by month + app + sector + cost_center
...
def calculate_direct_chargebacks(conn):
    ...
    Uses sector and Cost Center from applications_dim (enriched by ServiceNow).
    """
    cursor = conn.cursor()
    ...
    print(f"  INFO:  Found {len(months)} months with cost data")
    ...
    for month_start in months:
        ...
        # Calculate monthly costs per app with sector and Cost Center attribution
        cursor.execute("""
            WITH monthly_costs AS (
                SELECT
                    %s as month_start,
                    lc.app_id,
                    COALESCE(a.cost_center, 'Unknown') as cost_center,
                    a.sector_id,  -- NULL until CMDB enrichment (Phase 2)
                    a.owner_id,   -- NULL until CMDB enrichment (Phase 2)
                ...
                GROUP BY lc.app_id, a.cost_center, a.sector_id, a.owner_id
            )
...
def generate_chargeback_summary(conn):
    """Generate summary report of chargebacks by sector and Cost Center"""
    cursor = conn.cursor()
    ...
    cursor.execute("""
        SELECT 
            ...
            COUNT(CASE WHEN c.cost_center IS NOT NULL AND c.cost_center != 'Unknown' THEN 1 END) as apps_with_hcode
...
def validate_chargeback_coverage(conn):
    ...
    cursor.execute("""
        SELECT 
            ...
            COUNT(CASE WHEN cost_center IS NULL OR cost_center = 'Unknown' THEN 1 END) as missing_hcode,
            COUNT(CASE WHEN cost_center IS NOT NULL AND cost_center != 'Unknown' THEN 1 END) as with_hcode
        FROM chargeback_fact
    ...
    if hcode_pct < 80:
        print(f"    ⚠️  Cost Center coverage below 80%")
        print(f"       This is expected in test environments")
        print(f"       Production: Populate cost_center via ServiceNow or AppD tags")
    else:
        print(f"    ✅ Good Cost Center coverage")
    ...
    for row in cursor.fetchall():
        ...
        print(f"    {sector_name:<30} {row[1]:>6} records")
...
def run_chargeback_calculation():
    ...
    try:
        ...
        if records == 0:
            ...
            print("   No cost data available - this is expected when:")
            print("   • License Admin role is missing on all controllers")
            print("   • No license usage data has been collected yet")
            print("   Pipeline will complete successfully once license data is available")
            print("\n" + "=" * 70)
            print("✅ Chargeback Calculation Complete (No Data)")
            print("=" * 70)
            # Exit gracefully - this is not a fatal error
            sys.exit(0)
