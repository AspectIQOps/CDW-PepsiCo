...
def main():
    ...
    pipeline_steps = [
        # Phase 1: Extract core AppD data (apps, usage, costs)
        {
            ...
            'timeout': 2400,  # 40 minutes: 6 controllers × 5min API timeout + overhead
            'critical': True,
            'max_retries': 2,
    ...
    if has_servicenow:
        pipeline_steps.append({
            'script': 'snow_enrichment_fuzzy.py',
            'description': 'Phase 2: ServiceNow CMDB Enrichment (Fuzzy Matching)',
            'timeout': 600,
            'critical': False,  # Not critical - can run without enrichment
            'max_retries': 2,
            'reason': 'Adds CMDB data using fuzzy matching (SOW Section 2.4.3)'
        })
    else:
    ...
    # Phase 4: Chargeback calculation (NEW - aggregates costs by month/sector/cost_center)
    pipeline_steps.append({
    ...
    # Track results and phase timings
    results = []
    phase_timings = []
    ...
    for step in pipeline_steps:
        ...
        phase_timings.append((step['description'], phase_duration))
    ...
    print(f"{Colors.BLUE}Total Pipeline Runtime: ", end="")
    if hours > 0:
    ...
    print(f"\n{Colors.BLUE}Phase Breakdown:{Colors.NC}")
    for description, duration in phase_timings:
        ...
        print(f"  {description:<50} {duration_str:>10}")
